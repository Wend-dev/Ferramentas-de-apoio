...
Acusamos o recebimento de sua solicitação por meio do protocolo de n.º {protocol} e esclarecemos que o demonstrativo detalhado dos valores descontados em folha de pagamento deve ser solicitado diretamente ao seu órgão pagador/secretaria responsável.

Informamos que a operadora não possui acesso ao detalhamento individualizado dos valores efetivamente descontados em folha, uma vez que os repasses são realizados pelo próprio GDF de forma consolidada, sem a discriminação das rubricas específicas lançadas no contracheque do servidor.

Dessa forma, para obtenção do comprovante ou demonstrativo detalhado dos descontos, orientamos que entre em contato com o setor responsável em seu órgão. Permanecemos à disposição para prestar esclarecimentos adicionais no que estiver ao nosso alcance.`,
...
function generateText() {
  ...
  let genderPrefix;
  switch(gender) {
    case 'masculino': genderPrefix = 'Prezado'; break;
    case 'feminino':  genderPrefix = 'Prezada'; break;
    default:          genderPrefix = 'Prezado(a)'; break;
  }
